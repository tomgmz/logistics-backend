import { supabase } from '../../lib/supabase.js'
import * as GeneralManagerModel from '../../models/admin/general_manager.model.js'
import { BaseCreateDTO } from '../../types/user.types.js'

interface UpdateGeneralManagerDTO {
  first_name?: string
  last_name?: string
  middle_initial?: string | null
  suffix?: string | null
  username?: string
  email?: string
  phone?: string | null
}

export async function getAllGeneralManagers() {
  return GeneralManagerModel.findAll()
}

export async function getGeneralManagerById(userId: string) {
  const generalManager = await GeneralManagerModel.findById(userId)
  if (!generalManager) throw new Error('General Manager not found')
  return generalManager
}

export async function createGeneralManager(dto: BaseCreateDTO) {
  const e164Phone = dto.phone ? '+63' + dto.phone.slice(1) : undefined

  const { data: authData, error: authError } = await supabase.auth.admin.createUser({
    email:         dto.email,
    password:      dto.password,
    email_confirm: true,
    phone:         e164Phone ?? undefined,
    user_metadata: {
      role: 'general_manager',
      display_name: dto.username,
    }
  })
  if (authError) throw new Error(`Auth Error: ${authError.message}`)

  const userId = authData.user.id

  try {
    return await GeneralManagerModel.create(userId, dto)
  } catch (err: any) {
    console.error('General Manager creation failed, rolling back auth user...', err.message)
    const { error: rollbackError } = await supabase.auth.admin.deleteUser(userId)
    if (rollbackError) console.error('ROLLBACK FAILED. Orphan auth user ID:', userId)
    else console.log('Rollback successful.')
    throw new Error(`General Manager Creation Failed: ${err.message}`)
  }
}

export async function updateGeneralManager(userId: string, dto: UpdateGeneralManagerDTO) {
  if (dto.email) {
    const { error: authError } = await supabase.auth.admin.updateUserById(userId, {
      email: dto.email,
    })
    if (authError) throw new Error(`Auth update failed: ${authError.message}`)
  }
  return GeneralManagerModel.update(userId, dto)
}

export async function deleteGeneralManager(userId: string) {
  return GeneralManagerModel.remove(userId)
}
